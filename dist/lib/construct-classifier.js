...
export class ConstructClassifier {
    ...
    classifyWithLanguage(code, language) {
        const languagePattern = this.languagePatterns.find(pattern => pattern.name === language.toLowerCase());
        if (!languagePattern) {
    ...
    classifyWithGenericPatterns(code) {
     ...
     */
    getDetailedClassification(code, languageHint) {
     ...
     */
    addCustomPattern(language, construct, regex, confidence, details) {
