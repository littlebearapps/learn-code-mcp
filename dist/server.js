...
import { CallToolRequestSchema, ErrorCode, ListPromptsRequestSchema, ListToolsRequestSchema, McpError, GetPromptRequestSchema, } from '@modelcontextprotocol/sdk/types.js';
...
class LearnCodeMCPServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
            ...
            const lengthPreset = name.replace('explain_', '');
            ...
            try {
                switch (name) {
                    case 'explain_selection':
                        return this.handleExplainSelection(args);
    ...
    generateExplanationPrompt(code, length, language, filename, classification, workspaceContext) {
...
- [ ] Potential issues, optimizations, or architectural considerations`
    ...
    async handleExplainSelection(args) {
        const { code, length, language, filename, context } = args;
        if (!code || !length) {
        ...
        const explanationPrompt = this.generateExplanationPrompt(redactedCode, length, language, filename, classification, context);
...
Note: This prompt should be sent to your LLM for processing. The VS Code extension will handle the LLM communication and display the response.`;
    ...
    async handleClassifyConstruct(args) {
    ...
    async handleSetPreferences(args) {
